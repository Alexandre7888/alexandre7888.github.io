function ChatInterface({ user, onLogout, pendingJoinGroupId, onClearJoin }) {
    ...
    // URLs dos áudios (configure aqui suas URLs)
    const CALLING_SOUND_URL = "https://actions.google.com/sounds/obar/efeito-som-chamada-whatsapp.mp3";
    const RINGTONE_SOUND_URL = "https://actions.google.com/sounds/obar/toque-whatsapp.mp3";
    ...
    // Verificar dispositivo conflitante
    const checkDeviceConflict = async (callId) => {
        const callRef = db.ref(`calls/${callId}`);
        const snapshot = await callRef.once('value');
        const callData = snapshot.val();
        if (callData && callData.deviceId !== (navigator.userAgent || user.id) && callData.active) {
            showToastMessage("Outro dispositivo entrou na chamada! Encerrando esta...", "warning");
            return true;
        }
        return false;
    };
    ...
    // Override do sistema de notificação para usar áudios personalizados
    React.useEffect(() => {
        if (window.NotificationSystem) {
            ...
            window.NotificationSystem.playRingtone = () => {
                if (callStatus === 'calling' && callingAudioRef.current) {
                    callingAudioRef.current.play().catch(e => console.log("Erro ao tocar áudio de chamada:", e));
                } else if (incomingCall && ringtoneAudioRef.current) {
                    ringtoneAudioRef.current.play().catch(e => console.log("Erro ao tocar toque:", e));
        ...
        try {
            ...
            if (mediaData.fileType && mediaData.fileType.startsWith('image/')) {
                return (
                    <div className="mb-1">
                        {mediaUrl ? (
                            <img src={mediaUrl} className="rounded-lg max-w-full max-h-80 cursor-pointer object-contain" onClick={() => window.open(mediaUrl, '_blank')} alt={mediaData.fileName} />
                        ) : (
                            <div className="flex items-center gap-2 p-2 bg-gray-100 rounded-lg">
                                <div className="icon-image text-2xl text-gray-500"></div>
                                <div><p className="text-sm font-medium">{mediaData.fileName}</p><p className="text-xs text-gray-500">Carregando...</p></div>
                            </div>
                        )}
                        <button onClick={() => window.open(mediaUrl, '_blank')} className="mt-1 text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-lg">📥 Baixar</button>
                    </div>
                );
            }
            ...
            if (mediaData.fileType && mediaData.fileType.startsWith('video/')) {
                return (
                    <div className="mb-1">
                        {mediaUrl ? (
                            <video src={mediaUrl} controls className="rounded-lg max-w-full max-h-80" />
                        ) : (
                            <div className="flex items-center gap-2 p-2 bg-gray-100 rounded-lg">
                                <div className="icon-video text-2xl text-gray-500"></div>
                                <div><p className="text-sm font-medium">{mediaData.fileName}</p><p className="text-xs text-gray-500">Carregando vídeo...</p></div>
                            </div>
                        )}
                        <button onClick={() => window.open(mediaUrl, '_blank')} className="mt-1 text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-lg">📥 Baixar</button>
                    </div>
            ...
            return (
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border">
                    ...
                    <div className="flex-1"><p className="text-sm font-medium truncate">{mediaData.fileName}</p><p className="text-xs text-gray-500">{(mediaData.fileSize / 1024).toFixed(1)} KB</p></div>
                    <button onClick={() => { const link = document.createElement('a'); link.href = mediaUrl; link.download = mediaData.fileName; link.click(); }} className="p-2 bg-blue-500 text-white rounded-lg">📥</button>
                </div>
            );
        } catch (e) {
            return <p className="text-gray-800 mb-1 break-words">{msg.text}</p>;
        }
    };

    // ==================== FUNÇÕES DE CHAMADA P2P ====================
    ...
    const sendGlobalAudio = (audioBase64, duration) => {
        ...
        Object.keys(activeCalls).forEach(participantId => {
            db.ref(`users/${participantId}/global_audio`).set({ audio: audioBase64, duration, timestamp: Date.now(), sender: user.id });
        });
    ...
    React.useEffect(() => {
        ...
        globalAudioRef.on('value', (snapshot) => {
            const data = snapshot.val();
            if (data && data.sender !== user.id && professionalPanel) new Audio(data.audio).play();
        });
        ...
        const handleStatus = (snap) => {
            ...
            if (status) {
                if (status.state === 'active') {
                    ...
                    setOngoingGroupCall(null);
                    if ((Date.now() - status.timestamp < 5000) && (callStatus === 'connected' || callStatus === 'calling')) endCall(true);
                }
            } else setOngoingGroupCall(null);
        };
            ...
            const listener = messagesRef.limitToLast(1).on('child_added', (snapshot) => {
                ...
                if (!isFromMe && isRecent) {
                    ...
                    if (!isChatActive || document.hidden) {
                        window.NotificationSystem.show(`Nova mensagem de ${msg.senderName}`, msg.type === 'audio' ? '🎵 Áudio' : (msg.text?.substring(0, 50) || 'Mensagem'), chat.avatar);
                    }
                }
            });
    ...
    return (
        <div className="flex h-screen bg-gray-100 overflow-hidden relative">
                ...
                <div className="absolute inset-0 z-50 bg-black/50 flex items-center justify-center">
                    <div className="bg-white p-6 rounded-lg w-80 shadow-xl flex flex-col max-h-[80vh]">
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button onClick={() => { const val = document.getElementById('newContactId').value; if (callStatus) handleAddParticipantToCall(val); else handleAddContact(val); }} className="px-4 py-2 bg-[#00a884] text-white rounded">{callStatus ? 'Convidar' : 'Ir'}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* CALL MODAL - ESTILO WHATSAPP COM AVATAR REAL */}
            {(incomingCall || callStatus) && (
                <div className={`fixed z-50 transition-all duration-300 ease-in-out shadow-2xl overflow-hidden bg-gradient-to-b from-gray-900 to-black
                    ${isCallMinimized ? 'bottom-4 right-4 w-72 h-80 rounded-2xl border-2 border-white/20' : 'inset-0 flex flex-col items-center justify-center'}`}>
                        ...
                        <>
                            <div className="flex flex-col items-center mb-8">
                                <img 
                                    src={(() => {
                                        if (incomingCall) {
                                            const contact = chats.find(c => c.id === incomingCall.callerId);
                                            return contact?.avatar || user?.avatar;
                                        }
                                        return activeChat?.avatar;
                                    })() || user?.avatar}
                                    className="w-32 h-32 rounded-full mb-4 avatar-pulse object-cover"
                                    alt="Contact"
                                    onError={(e) => { e.target.src = user?.avatar; }}
                                />
                                <h2 className="text-2xl font-semibold text-white mb-1">
                                    {incomingCall ? (chats.find(c => c.id === incomingCall.callerId)?.name || incomingCall.callerId) : activeChat?.name}
                                </h2>
                                ...
                                {incomingCall ? (
                                    ...
                                    <>
                                        ...
                                        <button onClick={() => setIsCallMinimized(true)} className="call-button w-14 h-14 bg-gray-700 rounded-full flex items-center justify-center hover:bg-gray-600 shadow-lg">
                                            ...
                                            <div className="icon-arrow-left text-2xl text-white hidden"></div>
                                        </button>
                                    </>
                                )}
                            </div>
            ...
            <div className={`bg-white w-full md:w-[400px] border-r border-gray-200 flex flex-col ${activeChat ? 'hidden md:flex' : 'flex'}`}>
                ...
                <div className="flex-1 overflow-y-auto">
                    {chats.map(chat => (<div key={chat.id} onClick={() => openChat(chat)} className={`flex items-center p-3 cursor-pointer hover:bg-[#f5f6f6] ${activeChat?.id === chat.id ? 'bg-[#f0f2f5]' : ''}`}><img src={chat.avatar} className="w-12 h-12 rounded-full mr-3 object-cover" /><div className="flex-1 border-b border-gray-100 pb-3 h-full flex flex-col justify-center"><div className="flex justify-between items-baseline"><span className="text-gray-900 font-medium">{chat.name}</span>{chat.type === 'group' && chat.members?.[user.id] === 'admin' && (<span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full ml-2">Admin</span>)}</div><div className="text-sm text-gray-500 truncate w-48">{chat.type === 'group' ? 'Grupo' : 'Privado'}</div></div></div>))}
            ...
            {activeChat ? (
                <div className={`flex-1 flex flex-col h-full bg-[#efeae2] ${activeChat ? 'flex' : 'hidden md:flex'}`}>
                                            ...
                                            {isMediaMessage ? (
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <button onClick={() => copyToClipboard(msg.text, messageId)} className={`mt-2 text-xs flex items-center gap-1 px-2 py-1 rounded-lg transition-colors ${copiedMessageId === messageId ? 'bg-green-500 text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-100'}`}>
                                                        <div className="icon-copy text-xs"></div> {copiedMessageId === messageId ? 'Copiado!' : 'Copiar'}
                                                    </button>
                                                </div>
                                            ) : msg.type === 'audio' && (
                    ...
                    {Object.keys(uploadProgress).length > 0 && (
                        <div className="absolute bottom-20 left-4 right-4 bg-white rounded-lg shadow-lg p-2 border">
                            {Object.entries(uploadProgress).map(([name, progress]) => (
                                <div key={name} className="mb-1">
                                    <div className="flex justify-between text-xs text-gray-600 mb-1"><span className="truncate max-w-[200px]">{name}</span><span>{progress}%</span></div>
                                    <div className="w-full bg-gray-200 rounded-full h-1.5"><div className="bg-green-500 h-1.5 rounded-full transition-all duration-300" style={{ width: `${progress}%` }}></div></div>
                                </div>
                            ))}
                        </div>
                    )}
