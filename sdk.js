...
function _spinEl(){let el=document.getElementById(SPIN_ID);if(el)return el;el=document.createElement("div");el.id=SPIN_ID;el.innerHTML=`<div class="sp"><svg viewBox="0 0 50 50"><defs><linearGradient id="chSpinGrad" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#4f8cff"/><stop offset="1" stop-color="#7aa8ff"/></linearGradient></defs><circle class="bg" cx="25" cy="25" r="20"/><circle class="an" cx="25" cy="25" r="20"/></svg></div><div class="tx">Carregando...</div><div class="sb"></div>`;document.body.appendChild(el);return el}
...
const _s=(k,v)=>{try{localStorage.setItem(k,v)}catch(e){}};
const _r=k=>{try{return localStorage.getItem(k)}catch(e){return null}};
const _d=k=>{try{localStorage.removeItem(k)}catch(e){}};
...
showSpinner(t="Carregando...",s=""){_injectSpinCSS();const el=_spinEl();el.querySelector(".tx").textContent=t;el.querySelector(".sb").textContent=s||"";el.classList.add("show")}
...
async login(redirectUrl,spinner=true){this.clearSession();if(spinner)this.showSpinner("Abrindo login...","Você será redirecionado");try{const id=await this.createAuthId();this.saveSession(id);this.openPopup(id,redirectUrl);this.listenSSE(id);this._emit("started",{authId:id});return id}catch(e){if(spinner)this.hideSpinner();throw e}}
...
async createAuthToken(authId){const id=authId||this.readSession();if(!id)throw new Error("authId is required");return await this._get("/CreateAuthToken",{id})}
